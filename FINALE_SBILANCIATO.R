...
cross_plot(df, input = c( 'Sex','Age','Height','Weight','Year','Season','City','Sport','Event','Country','Continent','Sub.region'), target = "Medal", plot_type = "percentual",auto_binning=TRUE)
...
length(lista)
#metto i singoli status in coda a partire da lista[[100]]: lo status del datafrane 3 sarà lista[[103]]
...
# ESCLUDO LA VARIABILE event: non dà nessuna informazione aggiuntiva.
...
# bassa specificità che,associando r0 alla classe della NON conquista di medaglie, corrisponde
# a una bassa capacità di prevedere la classe con numerosità minore (r1) che è la classe sulla quale
...
# cambio tuning con una metrica più appropriata:ROC 
set.seed(1)
...
#dataframe per knn
knn_train<-data.frame(CountryRussia=ifelse(training$Country=="Russia", 1,0),Sub.regionNorthernEurope=ifelse(training$Sub.region=="Europe", 1,0),Year=training$Year,CountrySweden=ifelse(training$Country=="Sweden", 1,0),CountryNorway=ifelse(training$Country=="Norway", 1,0),CountryFinland=ifelse(training$Country=="Finland", 1,0),Height=training$Height,CitySarajevo=ifelse(training$City=="Sarajevo", 1,0),Age=training$Age,CountryEstonia=ifelse(training$Country=="Estonia", 1,0),Weight=training$Weight,CityGrenoble=ifelse(training$City=="Grenoble", 1,0),CityInnsbruck=ifelse(training$City=="Innsbruck", 1,0),CountryUK=ifelse(training$Country=="UK", 1,0),CityCalgary=ifelse(training$City=="Calgary", 1,0),SexF=ifelse(training$Sex=="F", 1,0),CityLakePlacid=ifelse(training$City=="LakePlacid", 1,0),CitySapporo=ifelse(training$City=="Sapporo", 1,0),Medal=training$Medal)
knn_test<-data.frame(CountryRussia=ifelse(test$Country=="Russia", 1,0),Sub.regionNorthernEurope=ifelse(test$Sub.region=="Europe", 1,0),Year=test$Year,CountrySweden=ifelse(test$Country=="Sweden", 1,0),CountryNorway=ifelse(test$Country=="Norway", 1,0),CountryFinland=ifelse(test$Country=="Finland", 1,0),Height=test$Height,CitySarajevo=ifelse(test$City=="Sarajevo", 1,0),Age=test$Age,CountryEstonia=ifelse(test$Country=="Estonia", 1,0),Weight=test$Weight,CityGrenoble=ifelse(test$City=="Grenoble", 1,0),CityInnsbruck=ifelse(test$City=="Innsbruck", 1,0),CountryUK=ifelse(test$Country=="UK", 1,0),CityCalgary=ifelse(test$City=="Calgary", 1,0),SexF=ifelse(test$Sex=="F", 1,0),CityLakePlacid=ifelse(test$City=="LakePlacid", 1,0),CitySapporo=ifelse(test$City=="Sapporo", 1,0),Medal=test$Medal)

#nzv
nearZeroVar(knn_train[,-19] , saveMetrics = TRUE) 
...
confusionMatrix(predglm, test$Medal)#solo r0:specificità=0
#
# IL PROBLEMA E' LA SOGLIA (la probabilità: che va tunata in funzione della grandezza d'interesse, in questo caso: specificità)
...
# utilizzo l'albero con la metrica d'interesse: specificità.
...
# per come è impostato il problema se voglio predire la classe minoritaria ( vincitori di medaglie)
# ho bisogno di specificità alta: scelta della soglia:


#scelgo cutoff:0.98 a cui corrisponde una specificità di 1-0.1379=0.86
...
# in altre parole, è sufficiente che la prob sia 0.02 per prevedere di vincere medaglie (r1)
...
# Specificity è aumentata a  0.9526 
...
profitMatrix[4, 4] <- 100 #è 100 volte più profittevole classificare una medaglia d'oro
...
probTr=as.matrix(probTr)  #probTr->riga 817 p(xj)#probabilità individuale di essere ogni classe j
...
colnames(zz)="predict"
# predict è la classe target che massimizza il profitto
